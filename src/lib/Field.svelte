...
<label class="form-control w-full max-w-xl">
  ...
  {#if field.type === "string"}
    {#if field.placeholder}
      <input type="text" class="input input-bordered" bind:value placeholder={field.placeholder} />
    {:else}
  ...
  {:else if field.type === "range"}
    <input type="range" min={field.min} max={field.max} class="range" step="1" bind:value />
  ...
  {:else if field.type === "file"}
    {#if field.accept}
      ...
      />
    {:else}
      <input type="file" class="file-input file-input-bordered w-full" on:change={parseText} />
    {/if}
  {/if}
</label>
